...
from src.resources.pydantic_schemas import VALID_MEAL_TYPES, RecipeSchema
...
@pytest.mark.asyncio
class TestRecipeGetByIdRoute:
    """Tests for GET /recipe/{recipe_id} endpoint."""

    @pytest.fixture
    async def test_recipe(self, db_session: AsyncSession, create_test_user: TestUser) -> TestRecipe:
    ...
    class TestSuccessScenarios:
        ...
        async def test_get_recipe_success(
            ...
            data = response.json()
            recipe_data = RecipeSchema(**data)
            assert recipe_data.meal_name == "Test Recipe"
            assert recipe_data.meal_type == VALID_MEAL_TYPES[0]
            assert len(recipe_data.ingredients) == 3
            assert "Ingredient 1" in recipe_data.ingredients
            assert len(recipe_data.instructions) == 3
            assert "Step 1" in recipe_data.instructions
        ...
        async def test_get_recipe_with_special_characters(
            ...
            data = response.json()
            recipe_data = RecipeSchema(**data)
            assert recipe_data.meal_name == "Cake with chocolate glaze"
            assert recipe_data.meal_type == VALID_MEAL_TYPES[3]
            assert "Wheat flour" in recipe_data.ingredients
            assert "Butter 82%" in recipe_data.ingredients
            assert "Bake at 180°C for 30 minutes" in recipe_data.instructions
            assert "Dust with powdered sugar & decorate" in recipe_data.instructions
